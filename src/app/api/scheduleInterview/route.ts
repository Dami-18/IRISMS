  ...
  try {
    ...
    const eventResponse = await calendar.events.insert({
      ...
      requestBody: {
        ...
        end: { dateTime: `${date}T${parseInt(time.split(":")[0]) + 1}:00`, timeZone: "Asia/Kolkata" },
        attendees: [{ email }],
        conferenceData: {
    ...
    await transporter.sendMail({
      ...
      return NextResponse.json({message: "Sent meet link", meetingLink}, {status: 200})
      
  } catch (err) {
    console.error("Error scheduling interview:", err);
    NextResponse.json({ message: "Failed to schedule interview" },{status: 500});
  }
}
